...
namespace GenericBug
{
	public static class Exceptions
	{
		...
		/// the action block are simply filtered to be sent as an error report, and never actually caught or handled. Filters all
		...
		/// GenericBug can halt the execution with <c>Environment.Exit(0);</c> if you configured it to do so with <paramref name="continueExecution"/>
		...
		/// <summary>
		/// Submits a bug report for the given exception. This function is useful for submitting bug reports inside a try-catch block.
		/// Note that this function uses the GenericBug configuration so it will use the pre-configured UI and submission settings.
		/// </summary>
        ...
        /// <summary>
        /// Submits a bug report for the given exception. This function is useful for submitting bug reports inside a try-catch block.
        /// Note that this function uses the GenericBug configuration so it will use the pre-configured submission settings.
        /// </summary>
        /// <param name="exception">The exception to submit as the bug report.</param>
        /// <param name="mode">temporarily set UIMode</param>
        public static void Report(Exception exception, UIMode mode)
        {
            UIMode oldMode = Settings.UIMode; //store old mode
            Settings.UIMode = mode; //set mode
            Report(exception);
            Settings.UIMode = oldMode; //revert back to original mode
        }
	}
}
