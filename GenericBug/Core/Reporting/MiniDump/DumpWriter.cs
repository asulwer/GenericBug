...
namespace GenericBug.Core.Reporting.MiniDump
{
	/// <summary>
	/// Sample usage:
	/// <code>
	...
	internal static class DumpWriter
	{
		...
		internal static bool Write(string minidumpFilePath)
		{
			if (Settings.MiniDumpType != MiniDumpType.None)
			{
				...
				{
					// ToDo: Create the minidump as a seperate process! Use this to deal with access errors: http://social.msdn.microsoft.com/Forums/en/csharpgeneral/thread/c314e6ca-4892-41e7-ae19-b3a36ad640e9
					// Bug: In process minidumps cause all sorts of access problems (i.e. one of them is explained below, debugger prevents accessing private memory)
		...
		private static bool Write(SafeHandle fileHandle, string dumpType)
		{
			...
			{
				throw GenericBugConfigurationException.Create(() => Settings.MiniDumpType, "Parameter supplied for settings property is invalid.");
			}
		}
