...
namespace GenericBug.Core.Util.Storage.ZipStorage.Zip
{
    ...
    partial class ZipEntry
    {
            ...
            get
            {
                ...
                if (this._RelativeOffsetOfLocalHeader > 0xFFFFFFFF)
                    ...
                    .Append(string.Format("          Timeblob: 0x{0:X8}\n", this._TimeBlob))
                        .Append(string.Format("              Time: {0}\n", GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.PackedToDateTime(this._TimeBlob)));
                                              ...
                                              Dictionary<String,Object> previouslySeen)
        {
            ...
            int signature = GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.ReadSignature(s);
            ...
            unchecked
            {
                ...
                zde._LastModified = GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.PackedToDateTime(zde._TimeBlob);
            ...
            {
                // UTF-8 is in use
                zde._FileNameInArchive = GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.Utf8StringFromBuffer(block);
            }
            else
            {
                zde._FileNameInArchive = GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.StringFromBuffer(block, expectedEncoding);
            }
            ...
            if (zde._commentLength > 0)
            {
                ...
                {
                    // UTF-8 is in use
                    zde._Comment = GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.Utf8StringFromBuffer(block);
                }
                else
                {
                    zde._Comment = GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.StringFromBuffer(block, expectedEncoding);
                }
            }
