...
namespace GenericBug.Core.Util.Web
{
	public class StreamUpload
	{
		...
		public StreamUpload Add(string name, string value)
		{
			...
			this.WriteToStream(this.outputStream, Encoding.UTF8, string.Format("Content-Disposition: form-data; name=\"{0}\"{1}{1}", name, Environment.NewLine));
		...
		public StreamUpload Add(Stream inputStream, string formName, string fileName, string contentType)
		{
			...
			this.WriteBoundaryToStream(this.outputStream, Environment.NewLine);
			this.WriteToStream(this.outputStream,Encoding.UTF8,string.Format("Content-Disposition: form-data; name=\"{0}\"; filename=\"{1}\"{2}", formName, fileName, Environment.NewLine));
