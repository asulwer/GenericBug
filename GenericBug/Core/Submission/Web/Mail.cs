...
namespace GenericBug.Core.Submission.Web
{
    /// <summary>
    ...
    public class MapiMailMessage
    {
        ...
        /// Sends the mail message.
        /// </summary>
        private void _ShowMail(bool ignore)
        {
            ...
            using (RecipientCollection.InteropRecipientCollection interopRecipients = _recipientCollection.GetInteropRepresentation())
            {
                ...
                if (!ignore)
                    _manualResetEvent.Set(); // Signal the creating thread (make the remaining code async)
        ...
        private void _LogErrorMapi(int errorCode)
        {
            ...
            System.Diagnostics.Debug.WriteLine("Error sending MAPI Email. Error: " + error + " (code = " + errorCode + ").");
        }
	...
	{
		public Mail(string connectionString) : base(connectionString)
		{
		}
		...
		*/
        ...
        public override bool Send(string fileName, Stream file, Report report, SerializableException exception)
        {
            ...
            {
                subject = string.Format("{0} ({1}): {2} from {3}",
                    report.GeneralInfo.HostApplication,
            ...
            {
                body = string.Format("{0}\n\r\n\r{1}\n\r\n\r{2}", this.CustomBody, report, exception);
            }
            else
            {
                body = string.Format("{0}\n\r\n\r{1}", report, exception);
            }

            MapiMailMessage message = new MapiMailMessage(subject, body);
            message.Recipients.Add(this.To);

            if (this.UseAttachment)
            {
                message.Files.Add(fileName);
            }
