...
namespace GenericBug.Core.Util.Storage.ZipStorage.Zip
{
    public partial class ZipEntry
    {
        ...
        private static bool ReadHeader(ZipEntry ze, System.Text.Encoding defaultEncoding)
        {
            ...
            int signature = GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.ReadEntrySignature(ze.ArchiveStream);
            bytesRead += 4;
            ...
            if (ZipEntry.IsNotValidSig(signature))
            {
                ...
                // workitem 10178
                GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.Workaround_Ladybug318918(ze.ArchiveStream);
            ...
            ze._LastModified = GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.PackedToDateTime(ze._TimeBlob);
            ...
            if (!ze._FileNameInArchive.EndsWith("/") && (ze._BitField & 0x0008) == 0x0008)
            {
                ...
                while (wantMore)
                {
                    ...
                    long d = GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.FindSignature(ze.ArchiveStream, ZipConstants.ZipEntryDataDescriptorSignature);
                    if (d == -1) return false;
        ...
        internal static void HandlePK00Prefix(Stream s)
        {
            ...
            uint datum = (uint)GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.ReadInt(s);
        ...
        private static void HandleUnexpectedDataDescriptor(ZipEntry entry)
        {
            ...
            {
                int sz = GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.ReadInt(s);
                if (sz == entry._CompressedSize)
                {
                    sz = GenericBug.Core.Util.Storage.ZipStorage.Zip.SharedUtilities.ReadInt(s);
